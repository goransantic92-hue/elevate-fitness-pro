export default {
  meta: {
    title: "Fitness blog za zaposlene muškarce 35+ — BUSY STRONG 90",
    description:
      "Praktični saveti za fitnes za zaposlene očeve u Dubaiju: nema vremena, mala energija, prekidali ste trening — i sistemi koji zaista rade.",
    keywords: [
      "fitness blog zaposleni muškarci",
      "nema vremena za trening",
      "dad bod fitnes",
      "konzistentnost u fitnesu",
      "trening za zaposlene očeve Dubai",
    ],
  },
  index: {
    eyebrow: "Resursi",
    headline: "Fitnes za",
    headlineHighlight: "zaposlene muškarce",
    subhead:
      "Bez hype-a. Bez predavanja u 5 ujutru. Pravi problemi — vreme, energija, konzistentnost, dad bod — i sistemi koji odgovaraju stvarnom životu.",
    minRead: "{{minutes}} min čitanja",
    readArticle: "Pročitaj članak",
  },
  post: {
    allArticles: "Svi članci",
    minRead: "{{minutes}} min čitanja",
    moreArticles: "Još članaka",
    ctaTitle: "Spremni za sistem koji odgovara vašem životu?",
    ctaSubhead: "90 dana. 3× nedeljno. 30–40 minuta kod kuće.",
    getProgram: "Uzmi program — {{price}}",
    startCoaching: "Započni coaching",
  },
  posts: {
    "no-time-too-tired-workout-busy-men": {
      title: "Nema vremena za trening? Previše umoran? Evo šta zaista radi za zaposlene muškarce",
      excerpt:
        "Nema vremena i previše ste umorni za vežbanje? Zašto zaposleni muškarci 35+ ne uspevaju u fitnesu — i kako 30-minutni domaći treninzi 3× nedeljno vraćaju energiju bez dodatnih sati u rasporedu.",
      metaTitle: "Nema vremena za trening? Previše umoran? 30-minutni sistem za zaposlene muškarce",
      metaDescription:
        "Nema vremena i previše ste umorni za vežbanje? Saznajte zašto zaposleni muškarci 35+ ne uspevaju u fitnesu — i kako 30-minutni domaći treninzi 3× nedeljno vraćaju energiju bez dodatnih sati u rasporedu.",
    },
    "fallen-off-wagon-fitness-consistency-busy-men": {
      title: "Prekinuli ste trening? Kako zaposleni muškarci grade konzistentnost bez motivacije",
      excerpt:
        "Povremeno trenirate, pa prestanete? Zašto zaposleni muškarci odustaju, kako prekinuti ciklus ponavljanja i izgraditi navike koje traju — bez oslanjanja na motivaciju.",
      metaTitle: "Prekinuli ste trening? Kako zaposleni muškarci grade konzistentnost bez motivacije",
      metaDescription:
        "Povremeno trenirate, pa prestanete? Zašto zaposleni muškarci odustaju, kako prekinuti ciklus ponavljanja i izgraditi navike koje traju — bez oslanjanja na motivaciju.",
    },
    "dad-bod-out-of-shape-after-kids-fitness": {
      title: "Dad bod i van forme posle dece? Kako ponovo postati jak bez početka ispočetka",
      excerpt:
        "Van forme posle dece? Praktičan vodič za muškarce 35+ da ponovo izgrade snagu, skinu kilograme i vrate telo — i energiju — koje ste imali pre nego što je život postao haotičan.",
      metaTitle: "Dad bod i van forme posle dece? Kako ponovo postati jak bez početka ispočetka",
      metaDescription:
        "Van forme posle dece? Praktičan vodič za muškarce 35+ da ponovo izgrade snagu, skinu kilograme i vrate telo — i energiju — koje ste imali pre nego što je život postao haotičan.",
    },
  },
  arabicBodyNotice:
    "Ovaj članak je trenutno dostupan samo na engleskom. Prebacite na engleski da biste pročitali ceo članak, ili se vratite uskoro za arapsku verziju.",
  articleBodyNotice:
    "Ovaj članak je trenutno dostupan samo na engleskom. Prebacite na engleski da biste pročitali ceo članak, ili se vratite uskoro za srpsku verziju.",
} as const;
