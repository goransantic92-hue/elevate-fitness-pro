...
/** Each slot done in either gym or home — matches contiguous week counting. */
export function isWeekFullyCompleteUnion(logs: WLog[], weekNumber: number): boolean {
  return SLOTS.every((slot) => {
    const gym = logs.some((l) => l.week_number === weekNumber && l.slot === slot && l.variant === "gym" && l.completed);
    const home = logs.some((l) => l.week_number === weekNumber && l.slot === slot && l.variant === "home" && l.completed);
    return gym || home;
  });
}

/** Weeks 1..n in order; stops at first week missing any slot (gym or home per slot). */
